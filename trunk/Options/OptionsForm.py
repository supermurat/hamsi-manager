...
class OptionsForm(MDialog):
    ...
    def checkVisibility(self, _showType):
        if _showType == "Normal":
            ...
            self.categories = [OptionsFormContent.General(self, _showType, ["isSaveActions"]),
                               ...
                               OptionsFormContent.SearchAndReplace(self, _showType)]
            if uni.isActiveDirectoryCover:
                self.categories.insert(1, OptionsFormContent.Cover(self, _showType,
                                                                   ["priorityIconNames", "isChangeExistIcon"]))
            ...
            self.categories = [OptionsFormContent.General(self, _showType, ["isSaveActions"]),
                               ...
                               OptionsFormContent.SearchAndReplace(self, _showType)]
            if uni.isActiveDirectoryCover:
                self.categories.insert(2, OptionsFormContent.Cover(self, _showType,
                                                                   ["priorityIconNames", "isChangeExistIcon",
            ...
            self.categories = [OptionsFormContent.General(self, _showType, ["isSaveActions"]),
                               OptionsFormContent.Correct(self, _showType, ["isEmendIncorrectChars",
    ...
    def reStart(self):
        answer = Dialogs.ask(translate("Options", "Please Restart"),
                             translate("Options",
                                       "In order to apply the changes you have to restart Hamsi Manager.<br>Do you want to restart now?"))
        ...
        if requestInfos[0] == "file":
            if requestInfos[1] == "image":
                ...
                filePath = Dialogs.getOpenFileName(translate("Options", "Choose Image"),
                                                   directory, str(
                        translate("Options", "Images")) + " " + uni.imageExtStringOnlyPNGAndJPG, 0)
    ...
    def pbtnDirectoryClicked(self):
        ...
        if requestInfos[0] == "directory":
            if requestInfos[1] == "exist":
                directory = fu.getRealPath(leValue.text())
    ...
    def pbtnDefaultValueClicked(self):
        ...
        if typeOfValue == "string":
            ...
            for y, info in enumerate(
                self.categories[categoryNo].stringSearchList[self.categories[categoryNo].typesOfValues[keyNo][1]]):
                value = value.replace(str(info), str(
                    self.categories[categoryNo].stringReplaceList[self.categories[categoryNo].typesOfValues[keyNo][1]][
                        y]))
            self.categories[categoryNo].values[keyNo].setText(str(value))
        elif typeOfValue == "options":
            self.categories[categoryNo].values[keyNo].setCurrentIndex(self.categories[categoryNo].valuesOfOptionsKeys[
                                                                          self.categories[categoryNo].typesOfValues[
                                                                              keyNo][1]].index(
                self.defaultValues[keyValue]))
    ...
    def createDefaultValueButton(self, _category, _typeOfValue, _keyValue, x):
        ...
        if _typeOfValue == "string":
            ...
            toolTips += str(_category.valuesOfOptions[_category.typesOfValues[x][1]][
                _category.valuesOfOptionsKeys[_category.typesOfValues[x][1]].index(self.defaultValues[_keyValue])])
                    ...
                    if category.visibleKeys.count(keyValue) > 0:
                        if category.typesOfValues[x] == "string":
                            ...
                            for y, info in enumerate(category.stringReplaceList[category.typesOfValues[x][1]]):
                                value = value.replace(str(info),
                                                      str(category.stringSearchList[category.typesOfValues[x][1]][y]))
                        elif category.typesOfValues[x][0] == "options":
                            value = category.valuesOfOptionsKeys[category.typesOfValues[x][1]][
                                category.values[x].currentIndex()]
                        ...
                        if uni.MySettings[keyValue] != value:
                            emendedValue = Settings.emendValue(keyValue, value, defaultValues[keyValue],
                                                               valueTypesAndValues[keyValue])
                            if emendedValue != value:
                                answer = Dialogs.ask(translate("Options", "Incorrect Value"),
                                                     str(translate("Options",
                                                                   "\"%s\" been set incorrectly.Are you want to set it automatically emend?")) % (
                                                         str(category.labels[x])))
                                if answer == Dialogs.Yes:
            ...
            if _category.visibleKeys.count(_keyValue) > 0:
                if _category.typesOfValues[x] == "string":
                    ...
                    for y, info in enumerate(_category.stringReplaceList[_category.typesOfValues[x][1]]):
                        value = value.replace(str(info),
                                              str(_category.stringSearchList[_category.typesOfValues[x][1]][y]))
                elif _category.typesOfValues[x][0] == "options":
                    value = _category.valuesOfOptionsKeys[_category.typesOfValues[x][1]][
                        _category.values[x].currentIndex()]
                ...
                if uni.MySettings[_keyValue] != value:
                    emendedValue = Settings.emendValue(_keyValue, value, defaultValues[_keyValue],
                                                       valueTypesAndValues[_keyValue])
                    if emendedValue != value:
                        answer = Dialogs.ask(translate("Options", "Incorrect Value"),
                                             str(translate("Options",
                                                           "\"%s\" been set incorrectly.Are you want to set it automatically emend?")) % (
                                                 str(_category.labels[x])))
                        if answer == Dialogs.Yes:
    ...
    def createOptions(self, _category):
        ...
        for x, keyValue in enumerate(_category.keysOfSettings):
            if _category.visibleKeys.count(keyValue) > 0:
                ...
                if _category.typesOfValues[x] == "string":
                    ...
                    for y, info in enumerate(_category.stringSearchList[_category.typesOfValues[x][1]]):
                        value = value.replace(str(info),
                                              str(_category.stringReplaceList[_category.typesOfValues[x][1]][y]))
                    _category.values[x].setText(str(value))
                elif _category.typesOfValues[x][0] == "options":
                    typeOfValue = "options"
                    ...
                    try: _category.values[x].setCurrentIndex(
                        _category.valuesOfOptionsKeys[_category.typesOfValues[x][1]].index(uni.MySettings[keyValue]))
                    except: pass  #pass for unknown values
                elif _category.typesOfValues[x][0] == "number":
                    typeOfValue = "number"
                    _category.values.append(MSpinBox())
                    _category.values[x].setRange(int(_category.valuesOfOptions[_category.typesOfValues[x][1]][0]),
                                                 int(_category.valuesOfOptions[_category.typesOfValues[x][1]][1]))
        ...
        _category.Panel.addStretch(1)
        if isNeededRestart:
            _category.Panel.addWidget(
                MLabel(translate("Options", "<font color=red>* :Requires a restart of Hamsi Manager.</font>")))
...
class EditDialog(MDialog):
    ...
    def apply(self):
        if self.typeOfValue == "string":
            ...
            #This Is Not Used (For only future)
            newValue = ""  #NotUsed
            self.parent().categories[self.categoryNo].values[self.keyNo].setCurrentIndex(
                self.parent().categories[self.categoryNo].valuesOfOptionsKeys[
                    self.parent().categories[self.categoryNo].typesOfValues[self.keyNo][1]].index(newValue))
        elif self.typeOfValue == "number":
            #This Is Not Used (For only future)
            newValue = ""  #NotUsed
