            ...
            if isContinueThreadAction:
                if InputOutputs.IA.isWritableFileOrDir(InputOutputs.currentDirectoryPath+"/"+str(_table.currentTableContentValues[realRowNo][1])):
                    if _table.isRowHidden(rowNo):
                        InputOutputs.IA.removeFileOrDir(InputOutputs.currentDirectoryPath+"/"+str(_table.currentTableContentValues[realRowNo][1]), True)
                        continue
                    ...
                    if _table.isColumnHidden(1)!=True and _table.item(rowNo,1).isSelected()==Universals.isChangeSelected or Universals.isChangeAll==True:
                        if str(_table.currentTableContentValues[realRowNo][1])!=unicode(_table.item(rowNo,1).text()).encode("utf-8"):
                            if unicode(_table.item(rowNo,1).text()).encode("utf-8").strip()!="":
                                ...
                                newFileName = InputOutputs.IA.moveOrChange(InputOutputs.currentDirectoryPath+"/"+str(_table.currentTableContentValues[realRowNo][1]),InputOutputs.currentDirectoryPath+"/"+unicode(_table.item(rowNo,1).text()).encode("utf-8"), InputOutputs.IA.getObjectType(InputOutputs.currentDirectoryPath+"/"+str(_table.currentTableContentValues[realRowNo][1])))
        ...
        for fileNo in range(startRowNo,self.Table.rowCount(),rowStep):
            ...
            for itemNo in range(0,2):
                if itemNo==0:
                    ...
                    newString = Organizer.emend(self.Table.currentTableContentValues[realFileNo][itemNo], InputOutputs.IA.getObjectType(InputOutputs.currentDirectoryPath+"/"+self.Table.currentTableContentValues[realFileNo][1]))
        ...
        for rowNo in range(startRowNo,self.Table.rowCount(),rowStep):
            ...
            for itemNo in range(self.Table.columnCount()):
                if itemNo==0:
                    ...
                    newString = Organizer.emend(unicode(self.Table.item(rowNo,itemNo).text(),"utf-8"), InputOutputs.IA.getObjectType(InputOutputs.currentDirectoryPath+"/"+self.Table.currentTableContentValues[realRowNo][1]))
