...
class SubFolderTable():
    ...
    def showDetails(self, _fileNo, _infoNo):
        TextDetails.TextDetails(InputOutputs.currentDirectoryPath+"/"+SubFolders.currentFilesAndFoldersValues[_fileNo][1],self.Table.isOpenDetailsOnNewWindow.isChecked())
    ...
    def save(self):
        returnValue = SubFolders.writeSubFolders(self.Table)
        self.Table.changedValueNumber = SubFolders.changedValueNumber
        return returnValue
    ...
    def refresh(self, _path):
        ...
        if Universals.isShowOldValues==True:
            ...
            for row in range(0,len(SubFolders.currentFilesAndFoldersValues)*2):
                ...
                n+=1
            self.Table.setRowCount(len(SubFolders.currentFilesAndFoldersValues)*2)
            ...
            self.Table.setRowCount(len(SubFolders.currentFilesAndFoldersValues))
        ...
        for fileNo in range(startRowNo,self.Table.rowCount(),rowStep):
            ...
            for itemNo in range(0,2):
                ...
                if str(SubFolders.currentFilesAndFoldersValues[realFileNo][itemNo])!=str(newString) and str(SubFolders.currentFilesAndFoldersValues[realFileNo][itemNo])!="None":
                    ...
                    self.Table.item(fileNo,itemNo).setToolTip(Organizer.showWithIncorrectChars(SubFolders.currentFilesAndFoldersValues[realFileNo][itemNo]).decode("utf-8"))
