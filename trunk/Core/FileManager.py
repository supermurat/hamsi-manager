...
class FileManager():
    ...
    def __init__(self, _parent):
        ...
        if Universals.windowMode==Universals.windowModeKeys[1]:
            ...
            tbarBrowserTools.addWidget(tools)
            _parent.addToolBar(Mt.TopToolBarArea, tbarBrowserTools)
            if Universals.isActivePyKDE4==True:
                toolsFull.setIconSize(MSize(16, 16))
                tbarBrowserToolsFull = MToolBar(_parent)
                tbarBrowserToolsFull.setWindowTitle(translate("FileManager", "Browser Tools (KDE4)"))
                tbarBrowserToolsFull.setObjectName(translate("FileManager", "Browser Tools (KDE4)"))
                tbarBrowserToolsFull.setIconSize(MSize(16,16))
                tbarBrowserToolsFull.addWidget(toolsFull)
                _parent.addToolBar(Mt.TopToolBarArea, tbarBrowserToolsFull)
                tbarLocationBar = MToolBar(_parent)
                tbarLocationBar.setWindowTitle(translate("FileManager", "Location Bar"))
                tbarLocationBar.setObjectName(translate("FileManager", "Location Bar"))
                tbarLocationBar.setIconSize(MSize(16,16))
                tbarLocationBar.addWidget(self.urlNavigator)
                _parent.addToolBar(Mt.TopToolBarArea, tbarLocationBar)
        else:
            ...
            dckwBrowserTools.setWidget(tools)
            ...
            if Universals.isActivePyKDE4==True:
                ...
                dckwBrowserToolsFull = MDockWidget(translate("FileManager", "Browser Tools (KDE4)"))
                dckwBrowserToolsFull.setObjectName(translate("FileManager", "Browser Tools (KDE4)"))
                dckwBrowserToolsFull.setWidget(toolsFull)
                dckwBrowserToolsFull.setAllowedAreas(Mt.AllDockWidgetAreas)
                dckwBrowserToolsFull.setFeatures(MDockWidget.AllDockWidgetFeatures)
                _parent.addDockWidget(Mt.LeftDockWidgetArea, dckwBrowserToolsFull)
                tbarLocationBar = MToolBar(_parent)
                tbarLocationBar.setWindowTitle(translate("FileManager", "Location Bar"))
                tbarLocationBar.setObjectName(translate("FileManager", "Location Bar"))
                tbarLocationBar.setIconSize(MSize(16,16))
