        ...
        try:
            ...
            if len(self.trueFiles) > 0 or len(self.falseFiles) > 0:
                if len(self.trueFiles) > 0:
                    ...
                    for song in self.trueFiles:
                        HBoxs.append(MHBoxLayout())
                        self.cbValues.append(MComboBox())
                        self.cbValues[-1].setObjectName("Value" + str(song[1]))
                        self.cbValues[-1].setEditable(True)
                        self.cbValues[-1].setToolTip(str(song[0]))
                        self.cbValues[-1].addItem(str(song[0]))
                        HBoxs[-1].addWidget(self.cbValues[-1])
                if len(self.falseFiles) > 0:
                    ...
                    for song in self.falseFiles:
                        HBoxs.append(MHBoxLayout())
                        self.cbValues.append(MComboBox())
                        self.cbValues[-1].setObjectName("Value" + str(song[1][1]))
                        self.cbValues[-1].setEditable(True)
                        self.cbValues[-1].setToolTip(str(self.parent().item(song[1][1], 1).text()))
                        for result in song[0]:
                            self.cbValues[-1].addItem(str(result.title))
                        HBoxs[-1].addWidget(self.cbValues[-1])
            ...
            if len(self.incorrectFiles) > 0:
                ...
                for song in self.incorrectFiles:
                    ...
                    self.cbValues[-1].setObjectName("Value" + str(song[1]))
                    self.cbValues[-1].setEditable(True)
                    self.cbValues[-1].setToolTip(str(self.parent().item(song[1], 1).text()))
                    self.cbValues[-1].addItem(str(song[0]))
                    HBoxs[-1].addWidget(self.cbValues[-1])
            for box in HBoxs:
