...
class Search(MDialog):
                ...
                try:
                    ...
                    Dialogs.showError(translate("SearchEngines", "An Error Occured"),
                                      str(translate("SearchEngines",
                                                    "Please retry the process.<br>If you receive the same error, please try the other search engines.<br><b>Error details:</b><br>%s")) % (
                                          str(errorDetails)))
    ...
    def checkIt(self, _tagsOfSong, _searchDepth):
        ...
        if _tagsOfSong[1].strip() != "":
            ...
            self.nullSongs.append(
                [[self.parent().item(_tagsOfSong[3], 1).text()], [_tagsOfSong[0]], [_tagsOfSong[1]], [_tagsOfSong[2]],
                 _tagsOfSong[3]])
            return
        if searchType == "Title":
            ...
            if len(titles) != 0:
                    ...
                    if str(title[2]).lower() == str(_tagsOfSong[1]).lower():
                        if str(title[5]).lower() == str(_tagsOfSong[2]).lower():
                            if str(title[7]).lower() == str(_tagsOfSong[0]).lower():
                                self.trueSongs.append(
                                    [_tagsOfSong[0], _tagsOfSong[1], _tagsOfSong[2], _tagsOfSong[3], _tagsOfSong[0],
                                     _tagsOfSong[1], _tagsOfSong[2]])
                                return
                ...
                self.falseSongs.append(
                    [artistNames, titleNames, albumNames, _tagsOfSong[3], _tagsOfSong[0], _tagsOfSong[1],
                     _tagsOfSong[2]])
            else:
                searchType = "Album"
        if searchType == "Album":
        ...
        if searchType == "Artist":
            ...
            if len(artists) != 0:
                ...
                self.songsOfArtist.append(
                    [artistNames, titleNames, albumNames, _tagsOfSong[3], _tagsOfSong[0], _tagsOfSong[1],
                     _tagsOfSong[2]])
            else:
                self.nullSongs.append(
                    [[self.parent().item(_tagsOfSong[3], 1).text()], [_tagsOfSong[0]], [_tagsOfSong[1]],
                     [_tagsOfSong[2]], _tagsOfSong[3]])
        ...
        while 4 > controlValue > 0:
            try:
                ...
                if str(errorDetails)[:15] == "HTTP Error 503:":
                    ...
                    raise ValueError(errorDetails)
        for result in returnedValues:
            values.append([result.score, result.release.id, result.release.title, result.release.asin,
                           (result.release.textLanguage, "/", result.release.textScript), result.release.types,
                           result.release.artist.id, result.release.artist.name])
    ...
    def getSongsFromTitle(self, _titleName):
        ...
        for result in returnedValues:
            values.append(
                [result.score, result.track.id, result.track.title, result.track.duration, result.track.releases[0].id,
                 result.track.releases[0].title, result.track.artist.id, result.track.artist.name])
        ...
        while 4 > controlValue > 0:
            try:
                ...
                if str(errorDetails)[:15] == "HTTP Error 503:":
                    ...
                    raise ValueError(errorDetails)
        values = [artist.id, artist.name, artist.sortName, artist.getUniqueName(), artist.type, artist.beginDate,
                  artist.endDate, ", ".join(t.value for t in artist.tags)]
    ...
    def getAlbumsOfArtist(self, _artistId, _searchDepth):
        ...
        for albumStateNo, albumState in enumerate(albumStates):
            if albumStateNo == 0 or (albumStateNo == 1 and _searchDepth > 4) or (
                        albumStateNo == 2 and _searchDepth > 7) or (albumStateNo == 3 and _searchDepth > 10):
                for albumTypeNo, albumType in enumerate(albumTypes):
                    if albumTypeNo <= _searchDepth:
                        ...
                        for release in artist.getReleases():
                            values.append([release.id, release.title, release.asin,
                                           (release.textLanguage, "/", release.textScript), release.types])
            ...
            if len(self.rows) != len(self.getSongsOfAlbum(albumId)):
                ...
                answer = Dialogs.ask(translate("SearchEngines", "Number Of Songs Are Different"),
                                     translate("SearchEngines",
                                               "The number of songs for the album you selected is not the same with the album you have verified.<br>Do you want to sort by the album anyway?"))
